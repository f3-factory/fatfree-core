...
class Web
{
     ...
     */
    public function mime(string $file, bool $inspect = false): string
    {
        if ($inspect) {
     ...
     * FALSE if none found
     * @param $list array|string|null
     **@return array|string|FALSE
     */
    public function acceptable(array|string|null $list = null): false|array|string
    {
        $accept = [];
        foreach (
            explode(',', str_replace(' ', '', @$_SERVER['HTTP_ACCEPT']))
            as $mime
        )
     ...
     */
    public function progress(string $id): false|int
    {
     ...
     */
    protected function _curl(string $url, array $options): array
    {
        $curl = curl_init($url);
     ...
     */
    protected function _stream(string $url, array $options): array
    {
        $eol = "\r\n";
        if (isset($options['proxy'])) {
     ...
     */
    protected function _socket(string $url, array $options): array
    {
        $eol = "\r\n";
        $headers = [];
        $body = '';
     ...
     */
    public function engine(string $arg = 'curl'): string
    {
        $arg = strtolower($arg);
        $flags = [
     ...
     */
    public function subst(array &$old, array|string $new): void
    {
        if (is_string($new))
            $new = [$new];
        foreach ($new as $hdr) {
     ...
     */
    public function request(string $url, ?array $options = null): array
    {
        $fw = Base::instance();
        $parts = parse_url($url);
        if (empty($parts['scheme'])) {
            ...
            throw new \Exception('Invalid URL scheme: '.$parts['scheme']);
        if (!is_array($options))
            $options = [];
        ...
        if ($result && isset($cache)) {
            if (preg_match(
                ...
                '/Cache-Control:.*max-age=(\d+),?.*'.preg_quote($eol).'/i',
                implode($eol, $result['headers']),
                $exp,
            ))
     ...
     */
    public function rss(string $url, int $max = 10, ?string $tags = null): false|array
    {
     ...
     */
    public function whois(string $addr, string $server = 'whois.internic.net'): false|string
    {
     ...
     */
    public function slug(string $text, string $separator = '-'): string
    {
        return trim(
            strtolower(
                preg_replace(
     ...
     */
    public function filler(int $count = 1, int $max = 20, bool $std = true): string
    {
        $out = '';
        if ($std)
...
if (!function_exists('gzdecode')) {
     ...
     **/
    function gzdecode(string $str): string
    {
        $fw = Base::instance();
