...
class SMTP extends Magic
{
    ...
    const
        ...
        E_SSL_Ext = 'OpenSSL extension not loaded',
    ...
    public function __construct(
        ...
        $this->host = strtolower(
            (($this->scheme = strtolower($scheme ?: '')) == 'ssl' ?
                'ssl' : 'tcp').'://'.$host,
        );
     ...
     * Fix header
     */
    protected function fixHeader(string $key): string
    {
        return str_replace(
            ' ',
            '-',
     ...
     */
    public function attach(string $file, ?string $alias = null, ?string $cid = null): void
    {
        if (!is_file($file))
        ...
        if ($this->attachments) {
            ...
            foreach ($this->attachments as $attachment) {
                if (is_array($attachment['filename']))
                    [$alias, $file] = $attachment['filename'];
                else
